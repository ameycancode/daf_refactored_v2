...
class CICDTrainingPipelineTest(TrainingPipelineTest):
    ...
    def __init__(self, environment: str, region: str = "us-west-2", 
        ...
        self.logger.info(f"CI/CD Enhanced Training Pipeline Test initialized for {environment}")
    ...
    def _get_environment_config(self) -> Dict[str, Any]:
        ...
        configs = {
            "dev": {
                "step_function_name": "energy-forecasting-dev-training-pipeline",
                "timeout_minutes": 30,
                "expected_profiles": ["RNN", "RN", "M", "S", "AGR", "L", "A6"]
            },
            "preprod": {
                "step_function_name": "energy-forecasting-preprod-training-pipeline", 
                "timeout_minutes": 45,
                "expected_profiles": ["RNN", "RN", "M", "S", "AGR", "L", "A6"]
            },
            "prod": {
                "step_function_name": "energy-forecasting-prod-training-pipeline",
                "timeout_minutes": 60,
                "expected_profiles": ["RNN", "RN", "M", "S", "AGR", "L", "A6"]
            }
        }
    ...
    def test_training_pipeline_cicd(self, timeout_minutes: Optional[int] = None) -> Dict[str, Any]:
        ...
        try:
            self.logger.info("="*80)
            self.logger.info("STARTING CI/CD ENHANCED TRAINING PIPELINE TEST")
            ...
            self.logger.info("="*80)
            self.logger.info("CI/CD ENHANCED TRAINING PIPELINE TEST COMPLETED SUCCESSFULLY")
    ...
    def _validate_step_function_exists(self, test_summary: Dict[str, Any]):
        ...
        self.logger.info("Validating Step Function exists...")
        ...
        try:
            ...
            self.logger.info(f"✓ Step Function validated: {self.env_config['step_function_name']}")
    ...
    def _start_training_execution(self, test_summary: Dict[str, Any], timeout_minutes: int) -> Dict[str, Any]:
        ...
        execution_name = f"training-test-{self.environment}-{int(time.time())}"
        if self.github_run_id:
        ...
        try:
            ...
            self.logger.info(f"✓ Training execution started: {execution_name}")
    ...
    def _monitor_training_execution(self, execution_arn: str, timeout_minutes: int, 
        ...
        self.logger.info(f"Monitoring training execution (timeout: {timeout_minutes} minutes)...")
        ...
        try:
            while datetime.now() < timeout_time:
                ...
                if not monitoring_result["execution_history"] or monitoring_result["execution_history"][-1]["status"] != current_status:
                    ...
                    self.logger.info(f"Training execution status: {current_status} (elapsed: {status_update['elapsed_time']})")
                ...
                if current_status in ["SUCCEEDED", "FAILED", "TIMED_OUT", "ABORTED"]:
                    ...
                    self.logger.info(f"Training execution completed with status: {current_status}")
                    break
    ...
    def _validate_training_results(self, monitoring_result: Dict[str, Any], 
        ...
        self.logger.info("Validating training pipeline results...")
        ...
        try:
            ...
            if monitoring_result["status"] == "SUCCEEDED":
                ...
                if not expected_profiles.issubset(actual_profiles):
                    ...
                    self.logger.info("✓ All expected profiles trained")
    ...
    def _generate_test_artifacts(self, test_summary: Dict[str, Any]):
        ...
        self.logger.info("Generating CI/CD test artifacts...")
        ...
        self.logger.info(f"Generated test results: {results_filename}")
